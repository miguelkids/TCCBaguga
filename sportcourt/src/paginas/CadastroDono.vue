    ...
    <main class="sc-container-sm sc-flex" style="flex: 1; align-items: center; justify-content: center; padding: 32px 16px;">
      <div class="sc-card" style="width: 100%; padding: 32px;">
        <div class="sc-flex-between" style="margin-bottom: 16px;">
          <span class="sc-badge sc-badge-amber">Perfil Dono de Arena</span>
          <router-link to="/escolher-perfil" class="sc-muted" style="font-size: 12px; text-decoration: none;">Alterar perfil</router-link>
        </div>

        <h1 class="sc-h2" style="margin-bottom: 6px;">Cadastre sua Arena no SportCourt</h1>
        <p class="sc-muted" style="margin-bottom: 24px;">Após a conta, você cadastrará as informações da sua quadra</p>
        ...
        <form @submit.prevent="handleCadastro">
          ...
          <button type="submit" class="sc-btn sc-btn-primary sc-btn-lg" :disabled="carregando">
            {{ carregando ? 'Criando conta...' : 'Continuar para Cadastro da Quadra →' }}
          </button>
        </form>
  ...
  methods: {
    async handleCadastro() {
      try {
        ...
        // Efetua login automático para prosseguir com onboarding da quadra
        const loginRes = await api.login(this.email, this.senha);
